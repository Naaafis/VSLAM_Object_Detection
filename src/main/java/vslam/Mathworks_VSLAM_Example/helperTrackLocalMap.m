...
% Check if there are no map points to process
if isempty(mapPointIdx)
    disp('No map points to process. Skipping local map tracking.');
    return;  % Early return to skip processing when there are no map points
end
